...
source("~/auth-aws.r") # contains human mortality database login credentials
...
if (!file.exists(here("_data/meps/h236.rds"))) {
    ...
    meps_post %>% write_rds(here("_data/meps/h236.rds"))
    ...
    meps_post <- read_rds(here("_data/meps/h236.rds"))
    meps_pre <- read_rds(here("_data/meps/h164.rds"))
}
...
df_ins_post <- ins_cov_post[[1]]

tmp <- 2:length(ins_cov_post) %>% map(~({
    df_ins_post <<- df_ins_post %>% left_join(ins_cov_post[[.x]],c("dupersid","month","year"))
}))
...
df_ins_post <- 
    ...
    mutate(type = case_when(
        ...
        ins == 1 & (peg == 1 | poe == 1 | tri == 1 | vap == 1) ~ 1, # Employer/TRICARE/VA
...
df_ins_pre <- ins_cov_pre[[1]]

tmp <- 2:length(ins_cov_pre) %>% map(~({
    df_ins_pre <<- df_ins_pre %>% left_join(ins_cov_pre[[.x]],c("dupersid","month","year"))
}))
...
lt =  demography::lifetable(tmp,series = "total", years = 2018)   # life table for background mortality
